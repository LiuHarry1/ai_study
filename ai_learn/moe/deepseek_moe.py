...
class MoELayer_GShard(layers.Layer):
    ...
    def call(self, inputs):
        logits = self.gate(inputs) #(32, 5, 4)
        top_k_values, top_k_indices = tf.nn.top_k(logits, k=self.top_k) #(32, 5, 2) , (32, 5, 2)

        expert_outputs = tf.zeros_like(inputs) #(32, 5, 512)
        for i in range(self.top_k):
            expert_id = tf.gather(top_k_indices, i, axis=-1) #(32, 5)
            weight = tf.gather(top_k_values, i, axis=-1)  #(32, 5)
            expert_output = tf.stack([self.experts[j](inputs) for j in range(self.num_experts)], axis=1) #(32, 5,5, 512)
            expert_output = tf.gather(expert_output, expert_id, batch_dims=1) #(32, 5, 5, 512)
            expert_outputs += tf.expand_dims(weight, -1) * expert_output
...
class MoELayer_DeepSeek(layers.Layer):
    ...
    def call(self, inputs):
        ...
        for i in range(self.top_k):
            ...
            expert_output = tf.gather(expert_output, expert_id, batch_dims=1)
            expert_outputs += tf.expand_dims(weight, -1) * expert_output
